...
func newRegistry() registry {
	return registry{studentList: make(map[int]student)}
}

func newStudent(name, phoneNum string) student {
	return student{name: name, phoneNumber: phoneNum}
}

func (r *registry) createStudent() {
	...
	case "y":
		r.currentId++
		newstudent := newStudent(name, phoneNumber)
		newstudent.magicCode = magicCode(phoneNumber, studentID)

		r.studentList[studentID] = newstudent
...
type student struct {
	name        string
	Name1       string `json:"name,omitempty" xml:""`
	phoneNumber string `json:"phNo"`
	// magicCode   int
}
